...
for i = 1:m
  ...
  %Compute gradient
  delta3 = a3(i, :) - transpose(yi);
  %delta3(1, num_labels)
  % Theta2(num_labels, hidden_layer_size + 1)
  delta2 = delta3*Theta2(:, [2:hidden_layer_size + 1]).*sigmoidGradient(z2(i, :));
  %delta2(1, hidden_layer_size)
  Theta2_grad = Theta2_grad + transpose(delta3)*a2(i,:)/m;
  Theta1_grad = Theta1_grad + transpose(delta2)*a1(i,:)/m;
endfor
